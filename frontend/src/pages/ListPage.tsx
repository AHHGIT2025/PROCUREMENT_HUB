...
//   // NEW — Project Master company filter (reuses the same /companies list
//   // fetched for Requests, since ListPage already fetches it for that type).
//   const [projCompanyFilter, setProjCompanyFilter] = useState("");
...
//       api.get(c.endpoint).then(r => setRows(r.data?.data ?? r.data ?? []));
//     }
//     if (type === "requests" || type === "projects") {
//       api.get("/companies").then(r => {
//         const list = Array.isArray(r.data) ? r.data : (r.data?.data ?? []);
//         setReqCompanies(list);
//       });
...
//     .filter(r => type !== "requests" || !dateFrom || new Date(r.createdAt) >= new Date(dateFrom))
//     .filter(r => type !== "requests" || !dateTo || new Date(r.createdAt) <= new Date(dateTo + "T23:59:59"))
//     .filter(r => type !== "projects" || !projCompanyFilter || r.companyName === projCompanyFilter)
//     .filter(r =>
//       type === "materials" ||
//       !search ||
//       Object.values(r).some(val =>
...
//   const userCompanies = type === "users" ? [...new Set(rows.map(r => r.company))] : [];
//   const userRoles     = type === "users" ? [...new Set(rows.map(r => r.role))]    : [];
//   const hasActiveFilters = !!(search || companyFilter || roleFilter || sourceFilter || reqCompanyFilter || dateFrom || dateTo || projCompanyFilter);
...
//           <>
//             <select className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 transition min-w-[200px]"
//               value={projCompanyFilter} onChange={e => setProjCompanyFilter(e.target.value)}>
//               <option value="">All Companies</option>
//               {reqCompanies.map((c: any) => (
//                 <option key={c.id ?? c.Id} value={c.name ?? c.Name}>{c.name ?? c.Name}</option>
//               ))}
...
//         {hasActiveFilters && (
//           <button
//             onClick={() => { setSearch(""); setCompanyFilter(""); setRoleFilter(""); setSourceFilter(""); setProjCompanyFilter(""); clearReqFilters(); }}
//             className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium bg-red-50 text-red-600 hover:bg-red-100 border border-red-100 transition ml-auto"
//           >
//             <X size={14} /> Clear filters
//           </button>
...
// // NEW — status pill options shown on the Purchase Requests page. 'val' must
// // match the exact string stored in r.status (see STATUS_FILTER_MAP above,
// // which is what dashboard card clicks resolve to via ?filter=... in the URL).
...
// // NEW — normalizes a status string for comparison (lowercase, strip spaces/
// // underscores/hyphens). This makes status filtering resilient to whatever
// // exact casing/spacing the backend actually stores (e.g. "PendingApproval",
// // "Pending_Approval", "pending approval" all match "Pending Approval").
...
//   // FIXED — `search` is now purely a free-text box (name/justification/etc).
//   // Status filtering is handled separately by `statusFilter` below, with an
//   // exact match against r.status, so a dashboard "Pending" click (or the
//   // on-page status pill) never accidentally matches unrelated text in other
//   // columns (e.g. a justification that happens to contain the word "pending").
//   const [search, setSearch]               = useState("");
...
//   function clearReqFilters() {
//     setSearch(""); setReqCompanyFilter(""); setDateFrom(""); setDateTo(""); setQuickDate(""); setStatusFilter("");
//   }
...
//     .filter(r => type !== "requests" || !dateFrom || new Date(r.createdAt) >= new Date(dateFrom))
//     .filter(r => type !== "requests" || !dateTo || new Date(r.createdAt) <= new Date(dateTo + "T23:59:59"))
//     // FIXED — status match decoupled from the free-text search box, and
//     // normalized so backend casing/spacing differences don't break it.
//     .filter(r => type !== "requests" || !statusFilter || normStatus(r.status ?? r.Status) === normStatus(statusFilter))
//     .filter(r => type !== "projects" || !projCompanyFilter || r.companyName === projCompanyFilter)
//     .filter(r =>
//       type === "materials" ||
//       !search ||
...
//   const userCompanies = type === "users" ? [...new Set(rows.map(r => r.company))] : [];
//   const userRoles     = type === "users" ? [...new Set(rows.map(r => r.role))]    : [];
//   const hasActiveFilters = !!(search || companyFilter || roleFilter || sourceFilter || reqCompanyFilter || dateFrom || dateTo || projCompanyFilter || statusFilter);
...
//             {/* NEW — status pill bar: exact-match filter, independent of the
//                 free-text search box. Reflects and drives `statusFilter`, so
//                 a dashboard "Pending" card click (which sets this via the URL
//                 on load) highlights the matching pill automatically. */}
...
//                 <button key={s.label} onClick={() => setStatusFilter(s.val)}
//                   className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
//                     normStatus(statusFilter) === normStatus(s.val)
//                       ? "bg-blue-600 text-white"
//                       : "bg-gray-50 border border-gray-200 text-gray-600 hover:bg-gray-100"
//                   }`}>
//                   {s.label}
...
// status pill options shown on the Purchase Requests page. 'val' must
// match the exact string stored in r.status (see STATUS_FILTER_MAP above,
// which is what dashboard card clicks resolve to via ?filter=... in the URL).
const REQUEST_STATUS_OPTIONS = [
  { label: 'All',      val: '' },
...
// normalizes a status string for comparison (lowercase, strip spaces/
// underscores/hyphens). This makes status filtering resilient to whatever
// exact casing/spacing the backend actually stores (e.g. "PendingApproval",
// "Pending_Approval", "pending approval" all match "Pending Approval").
function normStatus(s?: string) {
...
export default function ListPage({ type }: { type: string }) {
  ...
  // `search` is purely a free-text box (name/justification/etc).
  // Status filtering is handled separately by `statusFilter` below, with an
  // exact match against r.status, so a dashboard "Pending" click (or the
  // on-page status pill) never accidentally matches unrelated text in other
  // columns (e.g. a justification that happens to contain the word "pending").
  ...
  // hides deactivated users from the list by default. Toggle to
  // reveal them (e.g. to reactivate someone later).
  const [showInactive, setShowInactive]   = useState(false);
  ...
  // Project Master company filter (reuses the same /companies list
  // fetched for Requests, since ListPage already fetches it for that type).
  const [projCompanyFilter, setProjCompanyFilter] = useState("");
...
async function toggleUserStatus(userId: string, e: React.MouseEvent) {
  ...
  const filteredRows = rows
    ...
    .filter(r => type !== "requests" || !dateFrom || new Date(r.createdAt) >= new Date(dateFrom))
    .filter(r => type !== "requests" || !dateTo || new Date(r.createdAt) <= new Date(dateTo + "T23:59:59"))
    // status match decoupled from the free-text search box, and
    // normalized so backend casing/spacing differences don't break it.
    .filter(r => type !== "requests" || !statusFilter || normStatus(r.status ?? r.Status) === normStatus(statusFilter))
    .filter(r => type !== "projects" || !projCompanyFilter || r.companyName === projCompanyFilter)
    .filter(r =>
  ...
  // NEW — auto-opens a specific request's detail row when the URL has
  // ?view=<id> (e.g. from Admin/Requester Dashboard's "Recent Requests"
  // row click, which now navigates to `/purchase-requests?view=<id>` or
  // `/my-requests?view=<id>` instead of the old dead/unfiltered click).
  // Waits for `rows` to be loaded before trying to match, and only fires
  // once per id (guarded by the expandedId check) so it doesn't fight
  // the user if they manually collapse the row afterwards.
  useEffect(() => {
    if (type !== 'requests') return;
    const viewId = searchParams.get('view');
    if (!viewId || expandedId === viewId) return;
    const match = rows.find(r => (r.id ?? r.Id) === viewId);
    if (match) toggleExpandRequest(viewId);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, rows, type]);
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex gap-3 flex-wrap items-center">
        ...
        {type === "requests" && (
          <>
            ...
            {/* status pill bar: exact-match filter, independent of the
                free-text search box. Reflects and drives `statusFilter`, so
                a dashboard "Pending" card click (which sets this via the URL
                on load) highlights the matching pill automatically. */}
            <div className="flex items-center gap-1.5 flex-wrap w-full mt-1">
