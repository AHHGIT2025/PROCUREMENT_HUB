...
namespace Procurement.Api.Services.Integration
{
    ...
    public class SupplierSyncService
    {
        ...
        public async Task<SupplierSyncResult> SyncAsync(IErpConnector connector, string sourceType)
        {
            ...
            var now = DateTime.UtcNow;
            // ✅ FIX: A branch can have multiple OracleSourceMappings rows over time
            // (e.g. an old deactivated row plus a new active one, or separate
            // Items/Suppliers rows). Without filtering IsActive here, a deactivated
            // row and an active row for the same BranchId both pass this filter,
            // causing ToDictionaryAsync to throw "An item with the same key has
            // already been added" when it tries to add the same BranchId twice.
            var branchToCompany = await _db.OracleSourceMappings
                .Where(m => m.OracleSource == connector.ConnectorName
                         && m.IsActive
                         && m.EffectiveFrom <= now
