...
export default function InternationalPOCreate() {
  ...
  // ── role-based visibility. Managers/System Admin/Purchase Manager
  // see every approved MR (they're the ones assigning work out). Everyone
  // else (e.g. Purchase Officer) only ever sees MRs assigned to them.
  const userRoles: string[] = user.roles ?? [];
  const isManagerRole = userRoles.some(r => ['System Admin', 'Manager', 'Purchase Manager'].includes(r));
  ...
  const isLinkedToMr = !!form.linkedPurchaseRequestId;
  // true only when this page was opened via the "Convert to PO" click from
  // the Procurement Queue (carries ?prId=... in the URL). Manual MR
  // selection via the search box below does NOT set this, so those users
  // still get the "Refresh / Change MR" button to pick a different one.
  const cameFromQueue = !!searchParams.get('prId');

  useEffect(() => {
    api.get('/companies').then(r => setCompanies(r.data?.data ?? r.data ?? [])).catch(console.error);
    api.get('/suppliers').then(r => setSuppliers(r.data?.data ?? [])).catch(console.error);
    // Uses the same queue the Procurement page already uses, which carries
    // assignedToId + canUpdatePO — needed to restrict an officer to only
    // their own assigned MRs (see approvedPrs filter below).
     api.get('/procurement/queue').then(r => setPurchaseRequests(r.data?.data ?? r.data ?? [])).catch(console.error);
  }, []);
  ...
  useEffect(() => {
    if (!form.linkedPurchaseRequestId) { setPrItems([]); setPullQty({}); setPullChecked({}); return; }
    loadPrItems(form.linkedPurchaseRequestId);
  }, [form.linkedPurchaseRequestId]);

  const loadPrItems = (prId: string) => {
    setLoadingPrItems(true);
    api.get(`/purchase-requests/${prId}/items`)
      .then(r => {
        const list = r.data?.data ?? [];
        setPrItems(list);
        const qtyDefaults: Record<string, number> = {};
  ...
  // Manually re-pull the latest remaining quantities for the linked MR —
  // useful if someone else converted part of this MR to another PO while
  // this page was open, so the "remaining" numbers here go stale.
  const refreshPrItems = () => {
    if (form.linkedPurchaseRequestId) loadPrItems(form.linkedPurchaseRequestId);
  };

  // Local POs are always QAR — no supplier is being paid in a
  // foreign currency for a local order, so this is locked instead of
  // being an easy-to-miss manual step for the officer.
  useEffect(() => {
    if (!form.isInternational) {
  ...
  // role-gated. Managers see every approved MR (status field
  // from the queue endpoint: "Approved" / "Oracle Ready" / "PO Issued", all
  // of which mean "approved or beyond"). Everyone else only sees MRs
  // assigned to them, and only while there's still unconverted quantity.
  const approvedPrs = purchaseRequests.filter((p: any) =>
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-2xl border-2 border-emerald-100 shadow-sm p-6 bg-gradient-to-br from-emerald-50/40 to-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {/* Currency is locked to QAR and shown read-only
                whenever the PO is Local. Only International POs get the
                free-choice currency dropdown. */}
            <div>
              <label className="text-xs font-medium text-gray-500 flex items-center gap-1">
