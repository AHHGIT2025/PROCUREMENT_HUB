...
namespace Procurement.Api.Controllers.IndentTransfer
{
    ...
    public class IndentTransferController : ControllerBase
    {
        ...
        // GET api/indent-transfer/transferred
        // PRs that have at least one item already transferred to Oracle,
        // shown with the Oracle Document ID and transfer date for reference/audit.
        [HttpGet("transferred")]
        public async Task<IActionResult> GetTransferred()
        {
            var oracleCompanyIds = await _db.Companies
                .Where(c => c.IsOracleIntegrated)
                .Select(c => c.Id)
            ...
            var list = await _db.PurchaseRequests
                .Where(x => x.IsActive && oracleCompanyIds.Contains(x.CompanyId))
                .OrderByDescending(x => x.CreatedAt)
                .Select(x => new
                {
                    id = x.Id,
                    ...
                    items = _db.PurchaseRequestItems
                        .Where(i => i.PurchaseRequestId == x.Id && i.OracleTransferredAt != null)
                        .OrderByDescending(i => i.OracleTransferredAt)
                        .Select(i => new
                        {
                            id = i.Id,
                            materialId = i.MaterialId,
                            ...
                            quantity = i.Quantity,
                            uom = i.Uom,
                            oracleDocumentId = i.OracleDocumentId,
                            oracleTransferredAt = i.OracleTransferredAt
                        })
                        .ToList()
                })
                .ToListAsync();
