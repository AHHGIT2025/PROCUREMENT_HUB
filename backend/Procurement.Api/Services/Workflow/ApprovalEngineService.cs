...
namespace Procurement.Api.Services.Workflow
{
    ...
    public class ApprovalEngineService : IApprovalEngineService
    {
        ...
        // ── START WORKFLOW ──────────────────────────────────────────────
        // CHANGED: now sends a notification to the first approver after
        // the instance is created. Nothing else in this method changed.
        public async Task<EngineResult> StartWorkflowAsync(Guid prId)
        {
            var pr = await _db.PurchaseRequests.FirstOrDefaultAsync(p => p.Id == prId);
            if (pr == null) return Fail("Purchase request not found.");
            ...
            // NEW — notify the first approver that a request is waiting for
            // them. Works for any role name (Manager, TC-GM, SW-GM,
            // HTC-Manager, etc.) since it just notifies whoever
            // ResolveApproverAsync resolved — no role-name-specific logic.
            await SendApproverNotificationAsync(
                assignedTo.Value,
                "New Approval Request",
                $"Purchase request '{pr.RequestNumber}' is waiting for your approval at step '{firstStep.Name}'.");
        ...
        // ── PROCESS ACTION ──────────────────────────────────────────────
        // CHANGED: now sends a notification to the next approver when the
        // workflow advances to a new step. Reject/Return/Fully-Approved
        // notification calls (to the requester) are unchanged.
        public async Task<EngineResult> ProcessActionAsync(
            Guid instanceId, Guid userId, string action, string? comments)
        {
            var instance = await _db.ApprovalInstances
            ...
            // NEW — notify the next approver. Works regardless of what
            // their role is named per company.
            await SendApproverNotificationAsync(
                nextUser.Value,
                "New Approval Request",
                $"Purchase request '{pr.RequestNumber}' is waiting for your approval at step '{nextStep.Name}'.");
        ...
        // ═══════════════════════════════════════════════════════════════
        // International PO Approval methods.
        // ═══════════════════════════════════════════════════════════════

        // CHANGED: now sends a notification to the first PO approver.
        public async Task<EngineResult> StartPoWorkflowAsync(Guid poId)
        {
            var po = await _db.InternationalPurchaseOrders.FirstOrDefaultAsync(p => p.Id == poId);
            if (po == null) return Fail("International PO not found.");
            ...
            await SendApproverNotificationAsync(
                assignedTo.Value,
                "New PO Approval Request",
                $"International PO '{po.PoNo ?? "(unnumbered)"}' is waiting for your approval at step '{firstStep.Name}'.");
        ...
        // CHANGED: now sends a notification to the next PO approver when
        // the workflow advances to a new step.
        public async Task<EngineResult> ProcessPoActionAsync(
            Guid instanceId, Guid userId, string action, string? comments)
        {
            var instance = await _db.ApprovalInstances
            ...
            await SendApproverNotificationAsync(
                nextUser.Value,
                "New PO Approval Request",
                $"International PO '{po.PoNo ?? "(unnumbered)"}' is waiting for your approval at step '{nextStep.Name}'.");
        ...
        // CHANGED: now sends a notification to the next approver when the
        // workflow advances past a Store Verification step.
        public async Task<EngineResult> ProcessStoreVerificationAsync(
            Guid instanceId, Guid userId, List<StoreVerifyItemInput> items)
        {
            var instance = await _db.ApprovalInstances
        ...
        // IT Manager, etc.). Because SendApproverNotificationAsync just
        // notifies whatever UserId comes out of here, the notification fix
        // automatically works correctly for every company's own role names
        // — no role-name-specific code needed anywhere.
        private async Task<Guid?> ResolveApproverAsync(
            WorkflowStep step, Guid companyId, Guid requestedById)
        {
            if (step.ApproverType == "DEPARTMENT_MANAGER")
        ...
        // Notifies the REQUESTER (unchanged — used for reject/return/fully
        // approved/fulfilled-from-stock events).
        private async Task SendNotificationAsync(Guid prId, string title, string message)
        {
            var requestedById = await _db.PurchaseRequests
                .Where(p => p.Id == prId)
        ...
        // NEW — notifies the APPROVER that a new step is pending for them.
        // Takes a resolved UserId directly (not a role), so it works
        // identically no matter what that person's role is called in their
        // company (Manager, TC-GM, SW-GM, HTC-Manager, Budget Manager, etc.)
        // — the role-name resolution already happened in ResolveApproverAsync
        // before this is called.
        private async Task SendApproverNotificationAsync(Guid userId, string title, string message)
        {
