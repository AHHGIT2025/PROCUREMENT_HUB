...
//               <tbody>
//                 {(d.recentRequests ?? []).map((req: any) => (
//                  <tr key={req.id} onClick={() => navigate(`/my-requests?view=${req.id}`)}
//                     className="border-b border-gray-50 hover:bg-gray-50 transition cursor-pointer">
//                     <td className="py-3 px-1 font-semibold text-gray-800">{req.requestNumber}</td>
//                     <td className="py-3 px-1 text-gray-400 text-xs hidden sm:table-cell">
//                       {new Date(req.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' })}
...
// // FIXED — CEO and DCEO now see the same dashboard as System Admin.
// // isAdminRole is checked first in the routing logic at the bottom of this
// // file, so listing them here takes priority over their presence in
// // APPROVER_ROLES above (CEO stays in APPROVER_ROLES too — harmless, since
// // isAdminRole short-circuits before isApproverRole is ever checked for
// // these users). Confirm 'CEO' / 'DCEO' match the exact Role.Name strings
// // stored in the Roles table before deploying.
// function isAdminRole(roles: string[])       { return roles.some(r => ['System Admin', 'CEO', 'DCEO'].includes(r)); }
// function isApproverRole(roles: string[])    { return roles.some(r => APPROVER_ROLES.includes(r)); }
// function isProcurementRole(roles: string[]) { return roles.some(r => PROCUREMENT_ROLES.includes(r)) && !isAdminRole(roles); }
...
// // consistent section header used across every dashboard card: small
// // tinted icon chip + title + optional count badge + optional right-side
// // action link. Keeps every card's header visually identical instead of
// // ad-hoc plain <h2> text.
// function SectionHeader({
...
// // icon-chip stat card — modern ManageEngine-style tile with a
// // colored icon badge, big number, trend-free footer note, and a subtle
// // hover lift. Fully clickable (keeps existing navigate() click-through).
// type IconStatCardProps = {
//   label: string;
...
// // donut chart for status distribution — the classic ManageEngine
// // ServiceDesk "requests by status" ring, with a centered total and a
// // compact legend. Purely additive/visual; reads the same statusChart data
// // the bar chart already consumes.
// const DONUT_COLORS = ['#3b82f6', '#f59e0b', '#10b981', '#ef4444', '#8b5cf6', '#14b8a6', '#94a3b8'];

// // maps a status display name (as it appears in statusChart / badges,
// // e.g. "Pending Approval") to the URL filter key ListPage expects
// // (e.g. "PendingApproval"). Mirrors STATUS_FILTER_MAP in ListPage.tsx but
// // in reverse, so clicking a graph segment routes to the same filtered view
// // as the stat cards and the on-page status pills.
// // FIXED — normalizes a status label (strips spaces/underscores/hyphens,
// // lowercase) before lookup, so donut/bar chart clicks work regardless of
// // the exact spacing/casing the backend sends in statusChart[].name (e.g.
// // "PendingApproval" vs "Pending Approval"). Previously this map required
// // an exact string match, which silently failed and fell through to the
// // unfiltered '/purchase-requests' whenever the backend's spacing differed
// // from what was hardcoded here.
...
// function statusNavPath(name: string) {
//   const key = STATUS_NAME_TO_URL_FILTER[normStatusKey(name)];
//   return key ? `/purchase-requests?filter=${key}` : '/purchase-requests';
// }
...
//   // same manager check used on the Procurement Queue page itself
//   // (ProcurementQueue.tsx), so a manager's dashboard gets manager-relevant
//   // actions (who needs assigning, team-wide queue shape) instead of the
//   // "my own tasks" view an individual officer sees.
//   const isManager = roles.some(r =>
...
//       {isManager ? (
//         <>
//           {/* manager-only: quick-glance queue shape (Pending vs
//               Issued vs Unassigned) so a manager can size up the team's
//               workload without opening the full queue page first. */}
//           {queue.length > 0 && (
//             <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
...
//           {/* manager-only: the actionable list a manager actually
//               needs on a dashboard — requests with nobody assigned yet,
//               one click away from the Assign flow on the Queue page. */}
//           <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
//             <SectionHeader
...
//         </>
//       ) : (
//         /* donut of just this person's own assigned tasks, broken
//             down by Assigned vs In Progress. myTasks is already filtered
//             to x.assignedToId === userId above, so this stays strictly
//             personal even though the stat cards nearby (Total/Pending/
//             Issued) show company-wide queue numbers. */
...
//       {/* donut chart of the requester's own request statuses only
//           (Draft/Pending/Approved/Rejected). Same visual treatment as the
//           admin dashboard's status chart, but scoped strictly to this
//           person's own numbers, already present in `d` for the stat cards
//           above — no new API data needed. */}
...
//       {/* donut of this approver's own action queue: pending vs
//           already approved. Uses the exact same two numbers already shown
//           in the stat cards above, just visualized. */}
//       {(myPending > 0 || (d.approvedCount ?? d.approvedRequests ?? 0) > 0) && (
//         <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
...
//             {/* % share breakdown, clickable per row */}
//             <div className="space-y-3">
//               {(() => {
//                 const total = statusChart.reduce((s, x) => s + (x.value ?? 0), 0) || 1;
//                 return statusChart.map((s: any, i: number) => {
...
//               <tbody>
//                 {recentRequests.map((req: any) => (
//                   <tr key={req.id} onClick={() => navigate(`/purchase-requests?view=${req.id}`)}
//                     className="border-b border-gray-50 hover:bg-gray-50 transition cursor-pointer">
//                     <td className="py-3 px-1">
//                       <p className="font-semibold text-gray-800">{req.requestNumber}</p>
//                       <p className="text-xs text-gray-400">
...
// const APPROVER_ROLES = [
//   'Manager', 'IT Manager', 'Budget Manager', 'Asset Manager',
//   'Finance Approver',   'CEO', 'DCEO', 'Approver',
//   'Department Manager', 'Procurement Officer', 'Procurement Manager', 'SW-DM-Design', 'SW-DM-EX', 'SW-DM-FUR',
//   'Company GM', 'Vice Chairman'
// ];
...
//   // NEW — some admin-tier roles (e.g. CEO, DCEO) are ALSO approvers in the
//   // workflow (they sit in APPROVER_ROLES too). For them, the company-wide
//   // AdminDashboard alone hides their own personal action items. This pulls
//   // in the same myPendingApprovals / pendingApprovalQueue fields already
//   // returned by the dashboard API (used by ApproverDashboard) and shows a
//   // "My Pending Approvals" card automatically — no separate role check or
//   // extra API call needed, and it silently does nothing for a plain System
//   // Admin who isn't also in APPROVER_ROLES.
//   const showMyApprovals = roles.some(r => APPROVER_ROLES.includes(r));
//   const myPending = d.myPendingApprovals ?? 0;
//   const pendingQueue: any[] = d.pendingApprovalQueue ?? [];
...
//       {/* NEW — My Pending Approvals: only rendered for admin-tier users who
//           are also approvers (e.g. CEO, DCEO). Same list/card treatment as
//           ApproverDashboard's own "Pending Approvals" card. */}
//       {showMyApprovals && (
//         <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
//           <SectionHeader
//             icon={<Clock size={14} />} iconBg="bg-red-50" iconColor="text-red-600"
//             title="My Pending Approvals" count={myPending} countColor="bg-red-100 text-red-600"
//             action={<button onClick={() => navigate('/approvals')} className="text-xs text-blue-600 hover:underline">View all</button>}
//           />
//           {myPending === 0 ? (
//             <div className="text-center py-8 text-gray-400">
...
import {
  FileText, Clock, CheckCircle2, XCircle, RotateCcw, FileEdit,
  Wallet, TrendingUp, ClipboardList, PackageCheck, ListChecks,
  Building2, ArrowUpRight, UserPlus2, Globe2,
} from 'lucide-react';
...
function AdminDashboard({ d, name, roles }: { d: any; name: string; roles: string[] }) {
  ...
  // NEW — "Total PO" overview, replacing the old Status Breakdown section
  // (which just repeated the same Purchase Request status numbers already
  // shown in the stat cards and donut above). Pulled from the PO Management
  // module (/api/international-po) — same endpoint that page itself uses —
  // so this reflects actual PO records, not MR/request statuses.
  const [poList, setPoList] = useState<any[]>([]);
  const [loadingPo, setLoadingPo] = useState(true);

  useEffect(() => {
    api.get('/international-po')
  ...
  const PO_STATUS_LABELS: Record<string, string> = {
    Draft: 'Draft',
    QuotesCollected: 'Quotes Collected',
    SupplierSelected: 'Supplier Selected',
    Finalized: 'Finalized',
    SentToBright: 'Sent to Bright',
    Completed: 'Completed',
    Cancelled: 'Cancelled',
    Blocked: 'Blocked',
  };
  const PO_STATUS_COLORS: Record<string, string> = {
    Draft: '#94a3b8',
    QuotesCollected: '#3b82f6',
    SupplierSelected: '#6366f1',
    Finalized: '#8b5cf6',
    SentToBright: '#f59e0b',
    Completed: '#10b981',
    Cancelled: '#ef4444',
  ...
  const poStatusChart = Object.entries(
    poList.reduce((acc: Record<string, number>, p) => {
      const key = p.status ?? 'Unknown';
      acc[key] = (acc[key] ?? 0) + 1;
      return acc;
    }, {})
  ).map(([status, count]) => ({
    name: PO_STATUS_LABELS[status] ?? status,
    value: count,
    color: PO_STATUS_COLORS[status] ?? '#94a3b8',
  }));

  // NEW — PO Monthly Trend, same last-6-months line-chart treatment as the
  // Purchase Request "Monthly Trend" chart above, but computed client-side
  // from poList's createdAt (this endpoint doesn't pre-aggregate by month
  // the way the dashboard API does for requests).
  const MONTH_NAMES = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  const poMonthlyTrend = (() => {
    const now = new Date();
    const buckets: { key: string; name: string; count: number; value: number }[] = [];
    for (let i = 5; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
      buckets.push({ key: `${d.getFullYear()}-${d.getMonth()}`, name: `${MONTH_NAMES[d.getMonth() + 1]} ${d.getFullYear()}`, count: 0, value: 0 });
    }
    const byKey = Object.fromEntries(buckets.map(b => [b.key, b]));
    poList.forEach((p: any) => {
      const raw = p.createdAt ?? p.poDate;
      if (!raw) return;
      const d = new Date(raw);
      const key = `${d.getFullYear()}-${d.getMonth()}`;
      if (byKey[key]) {
        byKey[key].count += 1;
        byKey[key].value += p.totalAmount ?? 0;
      }
    });
    return buckets;
  })();
  ...
  return (
    <div className="space-y-7 pb-8">
      ...
      {/* NEW — replaces the old "Status Breakdown" section, which just
          repeated the same Purchase Request status counts already shown in
          the stat cards and the donut above. This instead surfaces PO
          Management data (a different data source — /api/international-po)
          so the dashboard doesn't show the same numbers four times over. */}
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        <SectionHeader
          icon={<Globe2 size={14} />} iconBg="bg-indigo-50" iconColor="text-indigo-600"
          title="PO Management Overview"
          action={<button onClick={() => navigate('/international-po')} className="text-xs text-blue-600 hover:underline">View all</button>}
        />
        {loadingPo ? (
          <div className="h-48 flex items-center justify-center text-gray-300 text-sm">Loading...</div>
        ) : poList.length === 0 ? (
          <div className="h-48 flex items-center justify-center text-gray-300 text-sm">No POs yet</div>
        ) : (
          <div className="grid md:grid-cols-2 gap-6 items-center">
            <StatusDonut data={poStatusChart} onSliceClick={() => navigate('/international-po')} />

            {/* NEW — PO Monthly Trend now sits where the Total POs / Total
                PO Value stat cards used to be, next to the donut. */}
            <div>
              <div className="flex items-center gap-2 mb-3">
                <TrendingUp size={14} className="text-indigo-500" />
                <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">PO Monthly Trend (Last 6 Months)</h3>
              </div>
              <ResponsiveContainer width="100%" height={200}>
                <LineChart data={poMonthlyTrend} margin={{ top: 4, right: 4, left: -15, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
                  <XAxis dataKey="name" tick={{ fontSize: 10, fill: '#94a3b8' }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: '#94a3b8' }} axisLine={false} tickLine={false} allowDecimals={false} />
                  <Tooltip content={<ChartTip />} />
                  <Legend wrapperStyle={{ fontSize: '11px' }} />
                  <Line type="monotone" dataKey="count" name="PO Count" stroke="#6366f1" strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
