...
namespace Procurement.Api.Services.Integration
{
    public class BrightOracleConnector : IErpConnector
    {
        ...
        // Combines UPDATED_DATE and CREATTION_DATE/CREATION_DATE, taking whichever is
        // more recent. Fixes a bug where a stale UPDATED_DATE (e.g. from a historical
        // bulk import) would cause a genuinely new record to be silently skipped by
        // the incremental sync, because NVL() always preferred UPDATED_DATE over
        // CREATTION_DATE whenever UPDATED_DATE was non-null - even if it was older.
        private static DateTime GreatestDate(DateTime? updated, DateTime? created)
        {
            if (updated.HasValue && created.HasValue)
                return updated.Value > created.Value ? updated.Value : created.Value;
            if (updated.HasValue)
                return updated.Value;
            if (created.HasValue)
                return created.Value;
            return DateTime.UtcNow; // shouldn't happen in practice - both dates null
        }
    ...
    WHERE GREATEST(NVL(UPDATED_DATE, CREATTION_DATE), NVL(CREATTION_DATE, UPDATED_DATE)) > :watermark
    ORDER BY GREATEST(NVL(UPDATED_DATE, CREATTION_DATE), NVL(CREATTION_DATE, UPDATED_DATE))", conn);
            ...
            {
                var updatedOrdinal = reader.GetOrdinal("UPDATED_DATE");
                var createdOrdinal = reader.GetOrdinal("CREATTION_DATE");

                DateTime? updated = reader.IsDBNull(updatedOrdinal) ? null : reader.GetDateTime(updatedOrdinal);
                DateTime? created = reader.IsDBNull(createdOrdinal) ? null : reader.GetDateTime(createdOrdinal);

                var updatedDate = GreatestDate(updated, created);
    ...
    WHERE GREATEST(NVL(UPDATED_DATE, CREATION_DATE), NVL(CREATION_DATE, UPDATED_DATE)) > :watermark
    ORDER BY GREATEST(NVL(UPDATED_DATE, CREATION_DATE), NVL(CREATION_DATE, UPDATED_DATE))", conn);
            ...
            {
                var updatedOrdinal = reader.GetOrdinal("UPDATED_DATE");
                var createdOrdinal = reader.GetOrdinal("CREATION_DATE");
