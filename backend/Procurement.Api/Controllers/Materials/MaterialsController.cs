...
namespace Procurement.Api.Controllers.Materials
{
    ...
    public class MaterialsController : Controller
    {
        ...
        // GET /api/materials/categories-for-company/{companyId}
        // Returns only categories that actually have items for this company —
        // powers the colorful category tabs on Create Request.
        [HttpGet("categories-for-company/{companyId:guid}")]
        public async Task<IActionResult> GetCategoriesForCompany(Guid companyId)
        {
            var data = await _context.Items
                .Where(i => i.CategoryId != null &&
                            _context.ItemCompanies.Any(ic =>
                                ic.ItemId == i.Id && ic.CompanyId == companyId))
                .GroupBy(i => i.CategoryId)
                .Select(g => new { CategoryId = g.Key!.Value, ItemCount = g.Count() })
                .Join(_context.ItemCategories.Where(c => c.IsActive),
                      g => g.CategoryId,
                      c => c.Id,
                      (g, c) => new
                      {
        ...
        public async Task<IActionResult> Create(CreateItemDto dto)
        {
            try
            {
                ...
                {
                    var groupName = await _context.ItemGroups
                        .Where(g => g.Id == dto.GroupId.Value)
                        .Select(g => g.Name)
                        .FirstOrDefaultAsync();

                    if (!string.IsNullOrEmpty(groupName))
                    {
                        var mappedCategoryId = await _context.ItemGroupCategoryMaps
                            .Where(m => m.OracleGroupName == groupName)
                            .Select(m => (Guid?)m.ItemCategoryId)
                            .FirstOrDefaultAsync();

                        if (mappedCategoryId.HasValue)
                            item.CategoryId = mappedCategoryId.Value;
                    }
                }
    ...
    : "None",
        ...
        public async Task<IActionResult> Update(Guid id, UpdateItemDto dto)
        {
            ...
            var warning = item.SourceType == "ORACLE"
                ? "?? This is an Oracle-synced item. Changes may be overwritten on next sync if Oracle data differs."
                : null;
